---
title: 2-6.VTable的渐进式加载
description: 深入解析VTable中ProgressProxy模块的实现原理与性能优化策略
date: 2025-04-12T10:00:00.000+00:00
lang: zh
type: note
duration: 10min
---
...
ProgressProxy 是 VTable 中 Scenegraph 模块的核心子模块，主要负责处理大数据量场景下的性能优化。这个模块通过控制场景节点的创建和更新过程，确保在大数据量下仍能保持流畅的渲染和交互体验。本文旨在通过阅读源码去探究ProgressProxy的实现特点。
...
配置的公式是例如：`Math.max(100, Math.ceil((table.tableNoFrameHeight * 2) / table.defaultRowHeight))`
...
经过阅读发现，渐进式渲染的核心逻辑在`setY`和 `setX`，分别用于处理垂直方向/水平方向上的的滚动和渐进式渲染。主要作用是根据滚动位置动态更新表格的行，确保当前可见区域的行被正确渲染，同时释放不可见区域的行以节省内存。这里以垂直方向的`setY`为例：
...
`dynamicSetY` 方法的设计核心在于通过动态调整表格的行渲染范围，实现高效、流畅的渐进式渲染。其原理基于滚动位置的实时计算和行偏移量的动态更新，确保当前可见区域的行被正确渲染，同时释放不可见区域的行以节省内存。

方法首先通过 `screenTop` 获取当前屏幕顶部行的信息，并根据 `isEnd` 参数判断是滚动中还是滚动结束，从而计算行偏移量 `deltaRow`。接着，调用 `move` 方法，根据行偏移量动态调整表格的行渲染范围，确保表格内容与滚动位置同步。在滚动结束时，方法会进一步计算垂直偏移量 `deltaY`，并更新 body 组的坐标，确保表格内容完整显示。

在大数据量滚动时，ProgressProxy模块会动态更新场景节点，滚动期间，按照滚动的方向，一部分单元格会更新其在场景树中的位置，并更新单元格内的图元，完成滚动效果。

## 总结

VTable 能够在大数据量场景下仍保持高性能和流畅的用户体验的秘密在于通过 ProgressProxy 模块控制表格只渲染可视范围内的数据量，并通过不断计算动态更新交互、滚动下的数据更新，也是表格组件性能优化的核心模块之一。

主要优化策略包括：

1. **节点数量控制**：通过`rowLimit`和`colLimit`限制同时维护的最大行列数
2. **缓冲区设计**：预加载两倍于可视区域的数据，确保滚动流畅
3. **动态节点更新**：根据滚动位置实时调整渲染的行列范围
4. **内存优化**：释放不可见区域的节点，减少内存占用
5. **高效渲染**：通过场景树的脏标记机制，只更新变化的部分

这些优化策略共同构成了VTable的渐进式加载机制，使其能够轻松应对大数据量表格的渲染挑战。
